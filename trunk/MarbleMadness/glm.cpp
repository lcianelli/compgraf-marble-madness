...
glmReadMTL(GLMmodel* model, char* name)
{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'm':
	  ...
	  fgets(buf, sizeof(buf), file);
	  sscanf(buf, "%s %s", buf, buf);
	  fileTexture = strdup(buf);
	  glmLoadTexture(fileTexture);
	break;
    default:
      /* eat up rest of line */
      fgets(buf, sizeof(buf), file);
...
GLuint glmLoadTexture(char* pathTexture)
{
	...
	FREE_IMAGE_FORMAT fifmt = FreeImage_GetFileType(pathTexture, 0);

	if(fifmt == FIF_UNKNOWN)
	{
		fifmt = FreeImage_GetFIFFromFilename(pathTexture);
	}

	FIBITMAP *dib;
	if(FreeImage_FIFSupportsReading(fifmt))
	{
			dib = FreeImage_Load(fifmt, pathTexture,0);
	}else
	{
		printf("No lo soporta\n");
	}
	...
	// Cada pixel va a ocupar 3 Bytes ... 24 bits
